...
async function renderCorpus(id,sessionId,opts){
  ...
  const entGroupsHTML=entRows.map(e=>{
    ...
    const kindLbl=_ENT_LABELS[e.kind]||esc(e.kind);
    // Mentions first, kind as a neutral pill last — same visual slot as a
    // doc's source-kind pill, but a distinct neutral color: an entity's
    // kind is a *classification*, not content provenance.
    const meta=`${n?`${n} mention${n===1?'':'s'} · `:''}<span class="doc-sk doc-sk-ent">${kindLbl}</span>`;
    return `<div class="doc-item doc-item--entity" data-entity-id="${e.id}" data-cat="${esc(e.kind)}"><div class="doc-hd"><span class="doc-tt">${esc(e.canonical_name)}</span><span class="doc-hd-right"><span class="doc-mt">${meta}</span><span class="doc-ar">▸</span></span></div></div>`;
  }).join('');
  // Wiki type-filter — chips appear only when ≥2 filterable categories are
  // present (a flat list of one kind needs no filter). Client-side show/hide
  // by data-cat; manifest stays under "All" only.
  const _catLabels={concept:'Concepts',person:'People',organization:'Companies',work:'Works',place:'Places'};
  const _catOrder=['concept','person','organization','work','place'];
  const _wikiCats=_catOrder.filter(c=>(c==='concept'&&conceptDocs.length)||entRows.some(e=>e.kind===c));
  const wikiFilterHTML=_wikiCats.length>=2
    ? `<div class="cv-wfilter" id="cv-wfilter"><button class="cv-wf cv-wf--on" data-cat="all">All</button>${_wikiCats.map(c=>`<button class="cv-wf" data-cat="${c}">${_catLabels[c]||c}</button>`).join('')}</div>`
    : '';
  const docItemHTML=(d)=>{
  ...
  // _backHref pairs with _backLabel for the corpus back-link. Restored here:
  // the can_write security commit (91fc4d2) dropped this declaration but left
  // its use in the header template below, which throws a ReferenceError in
  ...
  if(extractBtn)extractBtn.onclick=async()=>{
    ...
    try{
      ...
      const d=await r.json().catch(()=>({}));
      if(typeof handleQuotaError==='function'&&handleQuotaError(r,d)){extractBtn.disabled=false;extractBtn.textContent=o;return}
      if(!r.ok)throw new Error(d.detail||'Extract failed');
      const en=d.enriched||0,nm=d.new_mentions_total||0,rem=d.remaining||0;
      toast(en?`Extracted from ${en} doc${en===1?'':'s'}${nm?` · ${nm} new mention${nm===1?'':'s'}`:''}${rem?` · ${rem} left, run again`:''}`:'No new entities found','info');
      renderCorpusSafe(id);
    }catch(e){toast('Extract failed: '+e.message,'error');extractBtn.disabled=false;extractBtn.textContent=o}
  };
  ...
  const _wf=document.getElementById('cv-wfilter');
  if(_wf){
    const wd=document.getElementById('cv-wiki-docs');
    _wf.querySelectorAll('.cv-wf').forEach(btn=>btn.onclick=()=>{
      const cat=btn.dataset.cat;
      _wf.querySelectorAll('.cv-wf').forEach(b=>b.classList.toggle('cv-wf--on',b===btn));
      wd.querySelectorAll('.doc-item').forEach(it=>{
