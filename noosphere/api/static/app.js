/* Noosphere v5 — Create-first + Chat + Network */
const API='/api/v1';
let _gAnim=null,_lpAnim=null,_files=[],_corpora=[],_chatH=[];
...
async function route(){const h=location.hash||'#/';stopLP();
  ...
  await loadC();renderSB(h);
  if(h==='#/main'||h==='#/new')renderNew();
...
function renderSB(h){
  document.getElementById('sb-list').innerHTML=_corpora.length===0?'<div style="padding:6px 14px;font-size:11px;color:var(--tx3)">No corpora yet</div>'
    :_corpora.map(c=>`<a href="#/corpus/${c.id}" class="sb-corpus${h==='#/corpus/'+c.id?' active':''}"><span class="sb-dot" style="background:${cC(c.name)}"></span><span class="sb-cn">${esc(c.name)}</span></a>`).join('');
  document.getElementById('nav-net').classList.toggle('active',h==='#/network');
}
...
function renderNew(){
  ...
  document.getElementById('content').innerHTML=`
    <div class="new-page">
      <div class="new-title">What knowledge do you want to share?</div>
      <div class="new-sub">Add your knowledge to the Noosphere so agents can discover, query, and cite it.</div>
      <div class="new-options">
        <div class="new-opt" id="opt-write">
          ...
          <div class="new-opt-title">Write</div>
          <div class="new-opt-desc">Write content directly in the editor</div>
        </div>
        <div class="new-opt" id="opt-upload">
          ...
          <div class="new-opt-title">Upload</div>
          <div class="new-opt-desc">Upload .md, .txt files or import from URL</div>
        </div>
      </div>
      ${_corpora.length?`<div style="font-size:12px;color:var(--tx3)">Or select a corpus from the sidebar to manage it</div>`:''}
    </div>`;
  document.getElementById('opt-write').onclick=()=>{location.hash='#/write'};
  document.getElementById('opt-upload').onclick=()=>{location.hash='#/upload'};
}
  ...
  document.getElementById('content').innerHTML=`
    <div class="write-page">
      ...
      <div class="write-editor">
        <textarea id="w-body" placeholder="Start writing your knowledge here...

You can write in Markdown format. This will become a source in your corpus that agents can search and cite."></textarea>
      </div>
      <div class="write-bar">
        <div style="display:flex;gap:8px;align-items:center">
          <label style="font-size:12px;color:var(--tx2)">Corpus:</label>
          <select id="w-corpus" style="padding:4px 8px;border-radius:6px;border:1px solid var(--brd);background:var(--bg);color:var(--tx);font-size:12px">
            <option value="_new">+ Create new corpus</option>
            ${_corpora.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}
          </select>
        </div>
        <div style="display:flex;gap:8px">
          <button class="btn-ghost" onclick="location.hash='#/main'">Cancel</button>
          <button class="btn-primary" id="w-save">Save & Index</button>
        </div>
      </div>
    </div>`;
  document.getElementById('w-save').onclick=async()=>{
    ...
    const btn=document.getElementById('w-save');btn.disabled=true;btn.textContent='Saving...';
    let cid=document.getElementById('w-corpus').value;
    if(cid==='_new'){
      try{const r=await fetch(`${API}/corpora`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:title,description:'',author_name:'',tags:[],access_level:'public'})});const c=await r.json();cid=c.id}catch(e){btn.disabled=false;btn.textContent='Save & Index';return}
    }
    try{await fetch(`${API}/corpora/${cid}/upload`,{method:'POST',body:createMDFormData(title,body)})}catch(e){}
    btn.textContent='Indexing...';
    try{await fetch(`${API}/corpora/${cid}/index`,{method:'POST'})}catch(e){}
    location.hash='#/corpus/'+cid;
  };
}
function createMDFormData(title,body){const fd=new FormData();const content=`---\ntitle: ${title}\n---\n\n${body}`;fd.append('files',new Blob([content],{type:'text/markdown'}),title.replace(/[^a-zA-Z0-9]/g,'-')+'.md');return fd}
...
function renderUpload(){
  ...
  hideRP();_files=[];
  document.getElementById('content').innerHTML=`
    <div class="new-page" style="max-width:500px">
      <div class="new-title">Upload Knowledge</div>
      <div class="new-sub">Upload files or import from a URL</div>
      <div style="width:100%;text-align:left">
        <label class="fl">Corpus</label>
        <select id="u-corpus" class="fi" style="margin-bottom:8px">
          <option value="_new">+ Create new corpus</option>
          ${_corpora.map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join('')}
        </select>
        <div id="u-new-fields">
          <label class="fl">Name *</label><input type="text" id="u-name" class="fi" placeholder="e.g. My Blog, Research Notes" />
          <label class="fl">Description</label><input type="text" id="u-desc" class="fi" placeholder="What knowledge does this contain?" />
          <label class="fl">Tags</label><input type="text" id="u-tags" class="fi" placeholder="AI, startups (comma-separated)" />
        </div>
        <label class="fl">Upload files (.md, .txt)</label>
        <div class="dz" id="u-dz"><input type="file" id="u-fi" multiple accept=".md,.txt,.text,.html" hidden /><div class="dz-tx">Drop files here or <span class="dz-br">browse</span></div><div class="dz-ls" id="u-dl"></div></div>
        <label class="fl">Or paste a URL</label><input type="text" id="u-url" class="fi" placeholder="https://example.com/article" />
        <label class="fck"><input type="checkbox" id="u-idx" checked /> Auto-index after upload</label>
        <div class="reg-p"><strong>🌐 Join the Noosphere?</strong> Register so agents worldwide can discover your knowledge.<br/><label class="fck"><input type="checkbox" id="u-reg" checked /> Register to the Noosphere</label></div>
        <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:16px">
          <button class="btn-ghost" onclick="location.hash='#/main'">Cancel</button>
          <button class="btn-primary" id="u-submit">Publish</button>
        </div>
        <div class="hidden" id="u-status" style="font-size:12px;color:var(--acc);margin-top:8px"></div>
      </div>
    </div>`;
  const corpusSel=document.getElementById('u-corpus');
  corpusSel.onchange=()=>{document.getElementById('u-new-fields').style.display=corpusSel.value==='_new'?'':'none'};
  const dz=document.getElementById('u-dz'),fi=document.getElementById('u-fi');
  dz.onclick=()=>fi.click();dz.ondragover=e=>{e.preventDefault();dz.classList.add('drag-over')};dz.ondragleave=()=>dz.classList.remove('drag-over');dz.ondrop=e=>{e.preventDefault();dz.classList.remove('drag-over');addF(e.dataTransfer.files)};fi.onchange=()=>addF(fi.files);
  document.getElementById('u-submit').onclick=async()=>{
    const st=document.getElementById('u-status'),btn=document.getElementById('u-submit');btn.disabled=true;st.classList.remove('hidden');
    let cid=corpusSel.value;
    if(cid==='_new'){const nm=document.getElementById('u-name').value.trim();if(!nm){st.textContent='Name required';btn.disabled=false;return}st.textContent='Creating...';const tg=document.getElementById('u-tags').value.split(',').map(t=>t.trim()).filter(Boolean);try{const r=await fetch(`${API}/corpora`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:nm,description:document.getElementById('u-desc').value,tags:tg,access_level:'public'})});cid=(await r.json()).id}catch(e){st.textContent='Failed';btn.disabled=false;return}}
...
function renderNet(){
  if(_gAnim){cancelAnimationFrame(_gAnim);_gAnim=null}
  hideRP();
  document.getElementById('content').innerHTML=`<div class="nv-wrap"><canvas id="nv-cv" class="nv-canvas"></canvas><div class="nv-tt hidden" id="nv-tt"></div><div class="nv-bottom"><div class="composer" id="nv-comp"><textarea class="composer-input" id="nv-ci" placeholder="Ask the Noosphere..." rows="1"></textarea><div class="composer-toolbar"><div class="composer-left"></div><button class="composer-send" id="nv-send"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg></button></div></div><div id="nv-resp" style="margin-top:10px"></div></div></div>`;
  ...
  async function ask(){const q=ci.value.trim();if(!q)return;ci.value='';ci.style.height='auto';send.disabled=true;resp.innerHTML='<div style="font-size:13px;color:var(--tx3)">Thinking...</div>';
    ...
    catch(e){resp.innerHTML='<div style="font-size:13px;color:var(--tx3)">Failed to get response. Check LLM API keys in .env</div>'}send.disabled=false}
...
function drawGraph(){
  ...
  if(!_corpora.length){const p=cv.parentElement;if(p&&!p.querySelector('.empty')){const e=document.createElement('div');e.className='empty';e.style.position='absolute';e.style.top='40%';e.style.left='50%';e.style.transform='translate(-50%,-50%)';e.innerHTML='The Noosphere is empty.<br>Click <strong>+ New</strong> to add knowledge.';p.appendChild(e)}return}
  const W=cv.parentElement.clientWidth,H=cv.parentElement.clientHeight-cv.parentElement.querySelector('.nv-bottom').offsetHeight;
  ...
  let drag=null,hov=null,mp=null;const tt=document.getElementById('nv-tt');
  ...
  cv.onclick=e=>{if(!drag&&hov)location.hash='#/corpus/'+hov.id};
...
async function renderCorpus(id){
  if(_gAnim){cancelAnimationFrame(_gAnim);_gAnim=null}_chatH=[];
  ...
  ct.innerHTML=`
    ...
    <div class="chat-bottom"><div class="composer"><textarea class="composer-input" id="c-ci" placeholder="Chat with ${esc(c.name)}..." rows="1"></textarea><div class="composer-toolbar"><div class="composer-left"></div><button class="composer-send" id="c-send"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="12" y1="19" x2="12" y2="5"/><polyline points="5 12 12 5 19 12"/></svg></button></div></div></div>`;
  ...
  ci.addEventListener('input',()=>{ci.style.height='auto';ci.style.height=Math.min(ci.scrollHeight,120)+'px'});
  async function chat(){const msg=ci.value.trim();if(!msg)return;ci.value='';ci.style.height='auto';
    area.innerHTML+=`<div class="chat-msg user">${esc(msg)}</div>`;area.scrollTop=area.scrollHeight;
    send.disabled=true;area.innerHTML+=`<div class="chat-msg assistant" id="c-loading" style="color:var(--tx3)">Thinking...</div>`;area.scrollTop=area.scrollHeight;
    _chatH.push({role:'user',content:msg});
    try{const r=await fetch(`${API}/corpora/${id}/chat`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,history:_chatH,top_k:5})});const d=await r.json();
      document.getElementById('c-loading')?.remove();_chatH.push({role:'assistant',content:d.response});
      area.innerHTML+=`<div class="chat-msg assistant">${esc(d.response)}${d.citations&&d.citations.length?`<div class="chat-cites">${d.citations.map(c=>`<span class="chat-cite">${esc(c.title||'')}</span>`).join('')}</div>`:''}</div>`}
    catch(e){document.getElementById('c-loading')?.remove();area.innerHTML+=`<div class="chat-msg assistant" style="color:var(--tx3)">Failed. Check LLM API keys in .env</div>`}
    send.disabled=false;area.scrollTop=area.scrollHeight}
  send.onclick=chat;ci.onkeydown=e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();chat()}};
}

const ACC_MSG={public:'Discoverable by all agents worldwide.',private:'Only accessible via your personal endpoint.',token:'Requires access keys. (Token management coming soon)',paid:'Agents pay per query. (Stripe integration coming soon)'};
