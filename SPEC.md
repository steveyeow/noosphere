...
| Agent surfaces gaps, human fills them | **Interview:** `GET /corpora/{id}/gaps` ranks thin spots; `POST /corpora/{id}/interview` runs a gap-filling Q&A, saving answers as `user_capture`. Cloud gating: Free 3 sessions/day, Pro 50 (counted per session, on the opening turn); the proactive "questions for you" nudge (`GET /interview-queue`) is Pro-only. |
...
- [x] Corpus subscription (KB A subscribes to KB B increments) — **shipped with review-before-apply**. `peer_subscriptions` + `peer_subscription_runs` + `peer_subscription_pending` tables, CRUD REST, `peer_runner` (modes `ask` / `describe` / `new_documents`, cadence polling, loop detection, `source_kind='peer_subscription'` provenance). The runner **stages** each cycle to `peer_subscription_pending` (never writes straight into the corpus); the owner reviews a digest and approves (ingest, provenance retained) or discards. REST: `GET/POST .../subscriptions/{id}/pending[/approve|/discard]`; list_subscriptions exposes `pending_count`; UI shows a `N·review` affordance + review modal. Deduped by content hash against both ingested docs and still-pending rows. Remaining (own line in 4d): opt-in auto-apply for trusted subscriptions.
...
*Subscription review-before-apply — shipped.* A subscription pulls from a corpus the owner does not control, possibly paid and possibly drifting, so the trust boundary differs from same-owner ingest. Behavior is **propose, not apply**: each cycle's output is staged in `peer_subscription_pending` (status `pending`), and nothing enters the corpus until the owner reviews the digest and approves (ingest, `peer_subscription` provenance retained) or discards. This keeps "human always wins" intact across a boundary that direct same-owner ingest never crosses. Not yet built (follow-ups): per-cycle (vs. all-pending) approve granularity in the UI, a real notification surface beyond the `pending_count` badge, and opt-in auto-apply for trusted subscriptions (4d).
...
- [ ] Owner-approved auto-subscribe to peer KBs (consumes their increments) — the opt-in auto-apply path of the 4c subscription review design. Review-before-apply now ships as the default (4c); this remaining item is letting the owner mark a trusted subscription to skip the review step.
