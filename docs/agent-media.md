...
10. ~~**KBR v1 → v2 formula.**~~ **Resolved (v2a).** All four terms wired: `citation_pagerank` (0.4) · `query_retention` (0.3, from repeat `agent_id` over 30d) · `calibration_accuracy` (0.2, stubbed at 0.5 until feedback endpoint lands) · `satisfaction_rate` (0.1, `1 - refund_rate` over 90d). Nightly batch refresh via `POST /cron/refresh-kb-reputation`. Open: replace calibration stub with real feedback-based measurement.
11. ~~**Manifest auto-fill (LLM-proposed, owner-approved).**~~ **Resolved (backend).** `noosphere/core/manifest_autofill.py` generates `task_types` + `samples` + `description_suggestion` from corpus content. REST `/manifest/suggest` (proposal) + `/manifest/apply` (write). Post-indexing hook `autofill_if_empty` fills silently on first ingest — owner arrives to a pre-populated card, not a blank one. Web UI Capability card spec lives in `docs/capability-card-ui.md`; frontend deferred.
