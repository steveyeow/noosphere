...
# ── Cloud Connect routing (destination charge → creator) ──


@patch("noosphere.cloud.stripe_connect.connect_account_can_receive", return_value=True)
@patch("noosphere.cloud.db.get_user", return_value={"stripe_connect_account_id": "acct_creator"})
@patch("noosphere.api.routes._is_cloud", return_value=True)
@patch("noosphere.core.payments.stripe")
def test_checkout_cloud_routes_to_connect_account(
    mock_stripe, _mock_is_cloud, _mock_get_user, _mock_can_receive, client, paid_corpus
):
    """On cloud, the charge is a destination charge to the creator's Connect
    account, with the platform's 10% taken as an application fee."""
    from noosphere.core.db import get_conn

    conn = get_conn()
    conn.execute("UPDATE corpora SET owner_id=? WHERE id=?", ("owner-1", paid_corpus["id"]))
    conn.commit()

    mock_session = MagicMock()
    mock_session.id = "cs_cloud_1"
    mock_session.url = "https://checkout.stripe.com/cloud"
    mock_stripe.checkout.Session.create.return_value = mock_session

    with patch("noosphere.core.payments.STRIPE_SECRET_KEY", "sk_test_xxx"):
        r = client.post(
            f"/api/v1/corpora/{paid_corpus['id']}/checkout",
            json={"payer_email": "buyer@example.com"},
        )

    assert r.status_code == 200
    kwargs = mock_stripe.checkout.Session.create.call_args.kwargs
    pi = kwargs["payment_intent_data"]
    assert pi["transfer_data"]["destination"] == "acct_creator"
    assert pi["application_fee_amount"] == 50  # 10% of 500


@patch("noosphere.cloud.stripe_connect.connect_account_can_receive", return_value=False)
@patch("noosphere.cloud.db.get_user", return_value={"stripe_connect_account_id": "acct_creator"})
@patch("noosphere.api.routes._is_cloud", return_value=True)
def test_checkout_cloud_blocks_when_creator_cannot_receive(
    _mock_is_cloud, _mock_get_user, _mock_can_receive, client, paid_corpus
):
    """If the creator's Connect account can't receive transfers, the sale is
    refused (409) rather than collecting money the platform can't split."""
    from noosphere.core.db import get_conn
    ...
    assert "payout setup" in r.json()["detail"]
